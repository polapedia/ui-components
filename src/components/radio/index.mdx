...
- **Focus States**: To ensure clarity, the focus ring appears with a 4px offset from the radio border.
  - When **unchecked**, the focus ring is **black**.
  - When **checked**, the focus ring uses the **primary gradient**.
  - If in an **error state**, the focus ring is **red**.
...
- **Semantic Linking**: Uses `aria-describedby` to programmatically link the radio input with its `helperText`.
- **Validation State**: Uses `aria-invalid` to communicate error states to assistive technologies.
