...
export default function EmptyState(props: Readonly<EmptyStateProps>) {
  ...
  if (layout === 'card') {
    layoutClasses = `relative bg-white rounded-2xl shadow-lg border border-neutral-200 ${cardContainerClasses[size]}`;
  } else if (layout === 'horizontal') {
    layoutClasses = `flex flex-row items-center ${horizontalContainerClasses[size]}`;
  } else {
    layoutClasses = `flex flex-col items-center ${defaultContainerClasses[size]} mx-auto text-center`;
  }
  ...
  const imageSize = imageSizes[layout][size];
